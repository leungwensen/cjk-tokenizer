...
const combileStopWords = require('../get-stop-words');

const REGEXP_NON_ENGLISH = /[^A-Za-zéÉ'’_\-0-9@\.]+/;
...
function tokenizeEnglish(text, options) {
    ...
    const terms = {};
    const words = lang.map(text.split(REGEXP_NON_ENGLISH), str => lang.lc(str));
    const stopWords = combileStopWords(constant.STOP_WORDS.english, options.stopWords);
